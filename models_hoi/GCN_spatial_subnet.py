...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, Adj, coff_embedding=4, num_subset=1):
        ...
        for i in range(self.num_subset):
            self.conv_a.append(nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1),nn.BatchNorm2d(inter_channels),nn.ReLU()))
            self.conv_b.append(nn.Sequential(nn.Conv2d(in_channels, inter_channels, 1),nn.BatchNorm2d(inter_channels),nn.ReLU()))
            self.conv_d.append(nn.Sequential(nn.Conv2d(in_channels, out_channels, 1),nn.BatchNorm2d(out_channels),nn.ReLU()))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subset):
            ...
            A1 = torch.matmul(A1, A2) # N V V
            A1=A1/A1.max()
            A1=torch.softmax(A1,-2)
            A1 = A1 + A[i]
            A2 = x.reshape(N, C * T, V)
            z = torch.matmul(A2, A1).reshape(N, C, T, V)
            z=z/z.max()
            z=self.conv_d[i](z)
...
class GCN_spatial_subnet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.l9(x)
        x = self.l10(x)
        return x # output: N,C,T,out_channels. -> batch_size,out_channels,frame,MAX_N
